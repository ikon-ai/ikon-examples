...
private void RenderStartScreen(UIView view)
{
    view.Column(["h-screen flex flex-col bg-[#09090b]"], content: col =>
    {
        ...
        {
            var filter = _customerFilter.Value?.ToLowerInvariant() ?? "";
            ...
            scroll.Column(["gap-2 max-w-4xl mx-auto w-full"], content: ls =>
            {
                foreach (var customer in list)
                {
                    ...
                    ls.Button(["bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] rounded-lg p-4 text-left w-full"],
                        onClick: async () => StartChat(c),
                        content: v => v.Column(["gap-1"], content: col2 =>
                        {
                            col2.Text(["text-base font-medium text-white/70"], c.Name);
                            col2.Row(["gap-4"], content: r =>
                            {
                                if (!string.IsNullOrEmpty(c.Email))
                                    r.Text(["text-xs text-white/25"], c.Email);
                                if (!string.IsNullOrEmpty(c.Phone))
                                    r.Text(["text-xs text-white/25"], c.Phone);
                            });
                            if (!string.IsNullOrEmpty(c.Notes))
                                col2.Text(["text-xs text-white/15 mt-1 line-clamp-2"], c.Notes);
                        }));
                }
            });
        });
...
- Capture each customer in a local (`var capturedCustomer = customer;`) before the lambda — without it, the foreach variable closes over the last value and every row starts the same chat.
