...
When the user types a freeform prompt to start a thread/job/ticket, run a tiny `LLMModel.Default` LLM call to produce both a 3-8 word display title and a clean 1-2 sentence goal. Cheap, deterministic (`Temperature = 0`), one shot. Falls back to a string truncation if the call fails.
