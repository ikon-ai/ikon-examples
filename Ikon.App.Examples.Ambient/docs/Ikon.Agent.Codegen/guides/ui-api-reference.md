...
namespace Ikon.Parallax
  ...
  class UI
    ...
    // One-call per-client theme handling: syncs each joining client's theme, exposes the active theme as a bindable Current , and provides dark/light switching. The verbose form private readonly ClientReactive<string> _theme = new(Theme.Dark.ToThemeName()); // In Main: app.ClientJoinedAsync += async args => { if (string.IsNullOrEmpty(args.ClientContext.Theme)) { _theme.Value = Theme.Dark.ToThemeName(); await ClientFunctions.SetThemeAsync(Theme.Dark); } else { _theme.Value = (args.ClientContext.IsDarkTheme() ? Theme.Dark : Theme.Light).ToThemeName(); } }; private async Task ToggleThemeAsync() { var next = _theme.Value == Theme.Dark.ToThemeName() ? Theme.Light : Theme.Dark; await ClientFunctions.SetThemeAsync(next); _theme.Value = next.ToThemeName(); } becomes _theme = UI.UseTheme(); with _theme.Current bindable in views (e.g. a sun/moon icon) and _theme.ToggleAsync bindable to a button's onClick. When followClient is true (the default), a joining client that already has a theme keeps it and clients without one get defaultTheme ; when false, every joining client is forced to defaultTheme . Call once in Main, before clients join.
