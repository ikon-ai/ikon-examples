...
- `llm-title-and-goal-from-input` — polish raw user input into a titled spec.

## Voice & audio

- `voice-loop` — mic capture → STT → LLM → TTS playback.
- `push-to-talk-button` — CaptureButton in Hold mode with active styling.
- `tts-effect-chain-config` — add/remove/tune TTS audio effects live.
...
- `contact-start-screen` — pick an entity or skip into anonymous.
...
- `per-letter-glow-pulse` — animated wordmark, no JS.
- `dynamic-text-size-by-length` — scale type so short replies feel big.
...
- `avatar-with-image-fallback` — image-or-pulsing-box async avatar slot.
