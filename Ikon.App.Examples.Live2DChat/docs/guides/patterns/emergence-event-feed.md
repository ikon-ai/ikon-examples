...
You're shipping any Emergence pattern beyond a plain `await Emerge.Run<T>(…)` — the user needs to *see* the agent thinking. Also valuable for debugging your own prompt: when iterations balloon or tool calls go in circles, the feed shows it instantly.
