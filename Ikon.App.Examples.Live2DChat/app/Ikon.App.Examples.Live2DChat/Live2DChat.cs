...
public class Live2DChat(IApp<SessionIdentity, ClientParams> app)
{
    ...
    private readonly Reactive<int> _selectedModelIndex = new(1); // Default to Pina
    private readonly Reactive<int> _viewModeIndex = new(1); // Default to Portrait
    private readonly Reactive<bool> _settingsPanelOpen = new(false);

    // Chat messages history (use lock for thread safety)
    ...
    private const string SmallButtonStyle = "text-gray-700 text-sm px-3 py-2 rounded-lg font-medium bg-gray-100 hover:bg-gray-200 active:bg-gray-300 border border-gray-300 transition";
    private const string SectionStyle = "flex flex-col gap-2 p-4 rounded-xl bg-white/80 border border-gray-200";
    private const string LabelStyle = "text-xs text-gray-500";
    private const string InputStyle = "w-16 px-2 py-1 text-sm font-mono bg-white border border-gray-300 rounded text-gray-700";
    ...
    public async Task Main()
    {
        ...
        UI.Root(style: ["font-sans h-screen w-screen bg-white text-gray-900 overflow-hidden relative"], content: view =>
        {
            ...
            var transform = _viewModeIndex.Value switch
            {
            ...
            // Chat messages overlay (translucent bubbles on right side of avatar)
            // Use _chatMessagesVersion to trigger re-render when messages change
            ...
            view.Column(style: ["absolute bottom-32 right-8 top-20 w-[340px] z-10 flex flex-col justify-end gap-3 pointer-events-none"], content: view =>
            {
                foreach (var message in recentMessages)
                {
                    ...
                    {
                        // User message - cyan gradient bubble
                    ...
                    {
                        // AI message - dark glass bubble with cyan accent
                        view.Row(style: ["w-full justify-end"], content: view =>
                        {
                            view.Column(style: [
            ...
            {
                // Modern glassmorphism input container with cyberpunk styling
    ...
    {
        // Trigger re-render when effects change
        var _ = _ttsEffectsCount.Value;

        // Create a snapshot of effects under lock to avoid race conditions
        ...
        view.Column(style: ["px-5 pb-4"], content: view =>
        {
            ...
            view.Column(style: ["flex flex-col gap-2"], content: view =>
            {
                // Show active effects with parameters
                ...
                // Clear all button (only show if there are effects)
    ...
    {
        if (_ttsSpeaking.Value || string.IsNullOrWhiteSpace(_ttsText.Value))
        {
            return;
        }

        await SpeakTextAsync(_ttsText.Value);
    }
    ...
    private async Task SpeakTextAsync(string textToSpeak)
    {
        ...
        try
        {
            ...
            // Effects and analyzers for audio processing
        ...
        {
            // Log unexpected exceptions to help diagnose audio stopping issues
            Console.WriteLine($"[Live2DChat] Speech error: {ex.Message}");
        }
        finally
        {
            lock (_speechLock)
    ...
    private void RenderEffectParamRow(UIView view, int effectIndex, string paramKey, float paramValue, Action<int, string, float> updateAction)
    {
        ...
        {
            rowView.Text(style: [LabelStyle, "w-20"], text: paramKey);
            rowView.TextField(
                style: [InputStyle],
    ...
    private void AddSttEffect(string effectType)
    {
        var defaultParams = GetDefaultParams(effectType);
        var reactiveParams = EffectEntry.ToReactiveParams(defaultParams);
        var effect = CreateEffect(effectType, defaultParams);
        var entry = new EffectEntry(effectType, effect, reactiveParams);
        _sttEffects.Add(entry);
        _sttEffectsCount.Value = _sttEffects.Count;
    }

    private void RemoveSttEffect(int index)
    {
        if (index < 0 || index >= _sttEffects.Count)
        {
            return;
        }

        _sttEffects.RemoveAt(index);
        _sttEffectsCount.Value = _sttEffects.Count;
    }

    private void UpdateSttEffectParam(int index, string paramKey, float value)
    {
        if (index < 0 || index >= _sttEffects.Count)
        {
            return;
        ...
        var entry = _sttEffects[index];
        entry.Params[paramKey].Value = value;
        var newEffect = CreateEffect(entry.EffectType, entry.GetParamValues());
        entry.Effect = newEffect;
    }

    private void ClearSttEffects()
    {
        _sttEffects.Clear();
        _sttEffectsCount.Value = 0;
    }

    private void RenderSttEffectParams(UIView view, EffectEntry entry, int effectIndex)
    {
        foreach (var kvp in entry.Params)
        {
            RenderEffectParamRow(view, effectIndex, kvp.Key, kvp.Value.Value, UpdateSttEffectParam);
        }
    }
