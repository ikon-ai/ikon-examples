...
Showing a list of mixed-type artifacts (plans, code, notes, attachments), search hits, log entries, version history — anywhere the user wants to scan headers and then drill into one or two without leaving the list. The HashSet keying lets multiple cards stay open at once and keeps the row markup fully custom; reach for `view.AccordionSingle` / `view.AccordionMultiple` when the stock accordion look is fine.
...
- `ui-components` (top-level guide) — the `AccordionSingle` / `AccordionMultiple` primitives
