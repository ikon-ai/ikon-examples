...
<!-- checked-against: 5ebbc02520419be9 -->
...
- **`ReactiveScope.ClientId` is `int`, not `string`.** `app.Clients[id]?.Parameters` is the read path, and the one that works anywhere. There is **NO** `app.ClientSessionId`, **NO** `app.Clients.All`, **NO** `IClientCollection.Current`. `app.ClientParameters` does exist, but it resolves the *current* client from the ambient reactive scope — fine inside `UI.Root()`, and it throws in a background task, a timer or an event handler that has not opened a `ClientScope`. In an event handler, a single write to the joining client is `_x.SetFor(args.ClientSessionId, value)`; wrap with `using var _ = ReactiveScope.Use(new ClientScope(args.ClientSessionId))` when the whole handler body belongs to that client.
...
- **Don't iterate `app.Clients` to reset per-client state.** `IClientCollection<T>` is enumerable and also exposes `Ids` and `Count` beside the indexer, so `app.Clients.Ids` is the right handle when you genuinely want *every connected session*. It is the wrong set for game state: it holds whoever is connected right now, not your roster. To touch every player's `ClientReactive` value, walk your shared `_players` list and write to each id: `_selectedAnswer.SetFor(p.ClientId, null)`.
- **Give the `Reactive<T>` constructor an explicit initial value** — `new Reactive<GameStage>(GameStage.Lobby)`, `new Reactive<int>(0)`, `new ClientReactive<int?>((int?)null)` — so the starting stage is stated rather than inferred. Bare `new Reactive<T>()` does compile (an overload marker exists for it) and yields `default(T)`; that is right only where `default` genuinely is the starting value. The `ReactiveList<T>` family starts empty either way, so `private readonly ReactiveList<Player> _players = new();` takes no argument.
