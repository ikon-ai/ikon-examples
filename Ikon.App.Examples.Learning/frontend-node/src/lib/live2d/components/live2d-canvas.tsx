...
// Fixed canvas size for small containers (circular avatar) for consistent rendering
const SMALL_CANVAS_SIZE = 1000;
const SMALL_CONTAINER_THRESHOLD = 200;

// Helper to update model transform based on canvas size and server-provided values
function updateModelTransform(
  ...
  const baseScale = canvasHeight / dimensions.height;
  const finalScale = baseScale * scale;

  model.scale.set(finalScale);
  model.anchor.set(0.5, 0.5);

  // Position at canvas center, with vertical offset
  // offsetY adjusts which part of the model is shown (negative = show more head)
  model.x = canvasWidth / 2;
  model.y = canvasHeight / 2 - offsetY * canvasHeight;
}
...
const Live2DCanvasInner = memo(function Live2DCanvasInner(props: Live2DCanvasProps) {
  ...
  if (transformChanged) {
    ...
    if (model && dimensions && app) {
      const rect = containerRef.current?.getBoundingClientRect();
      const isSmallContainer = rect && (rect.width < SMALL_CONTAINER_THRESHOLD || rect.height < SMALL_CONTAINER_THRESHOLD);
      const canvasWidth = isSmallContainer ? SMALL_CANVAS_SIZE : window.innerWidth;
      const canvasHeight = isSmallContainer ? SMALL_CANVAS_SIZE : window.innerHeight;

      updateModelTransform(model, dimensions, scaleRef.current, offsetYRef.current, canvasWidth, canvasHeight);
      // Update base transform for camera movement
  ...
  // Store canvas size props in refs for access in resize observer
  const canvasWidthPropRef = useRef<number | undefined>(props.canvasWidth);
  const canvasHeightPropRef = useRef<number | undefined>(props.canvasHeight);
  canvasWidthPropRef.current = props.canvasWidth;
  canvasHeightPropRef.current = props.canvasHeight;
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      // Use explicit canvas size props if provided, otherwise auto-detect
      let canvasWidth: number;
      let canvasHeight: number;

      if (canvasWidthPropRef.current && canvasHeightPropRef.current) {
        // Use explicit dimensions from props
        canvasWidth = canvasWidthPropRef.current;
        canvasHeight = canvasHeightPropRef.current;
      } else {
        // Auto-detect based on container size
        const rect = container.getBoundingClientRect();
        const isSmallContainer = rect.width < SMALL_CONTAINER_THRESHOLD || rect.height < SMALL_CONTAINER_THRESHOLD;
        canvasWidth = isSmallContainer ? SMALL_CANVAS_SIZE : window.innerWidth;
        canvasHeight = isSmallContainer ? SMALL_CANVAS_SIZE : window.innerHeight;
      }
      ...
      if (model && dimensions) {
        updateModelTransform(model, dimensions, scaleRef.current, offsetYRef.current, canvasWidth, canvasHeight);
        // Update base transform for camera movement after resize
        baseTransformRef.current = {
          x: model.x,
          y: model.y,
          scale: model.scale.x,
  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      ...
      // Determine canvas size - use explicit props if provided, otherwise auto-detect
      let canvasWidth: number;
      let canvasHeight: number;
      ...
      try {
        ...
        updateModelTransform(model, modelDimensionsRef.current, scaleRef.current, offsetYRef.current, canvasWidth, canvasHeight);
...
const Live2DCanvasRenderer = memo(function Live2DCanvasRenderer({
  ...
  const canvasWidth = toFiniteNumber(node.props['canvasWidth']);
  const canvasHeight = toFiniteNumber(node.props['canvasHeight']);
