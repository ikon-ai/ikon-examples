...
public class MainMenuState(LearningApp outer) : ILearningState
{
    ...
    public void Render(UIView contentView)
    {
        ...
        contentView.Column(["gap-4 md:gap-5 pt-2 md:pt-0"], content: view =>
        {
            ...
            view.Box([LearningApp.Styles.GlassCardStrong, "p-5 md:p-6 rounded-3xl"], content: sectionView =>
            {
                sectionView.Column(["gap-4 md:gap-5"], content: col =>
                {
                    ...
                    col.Column(["gap-3 md:gap-4"], content: menuView =>
                    {
                        ...
                        menuView.Button(["default", $"justify-start {LearningApp.Styles.GetAccentGradient(theme)} text-white w-full py-4 md:py-5 rounded-2xl font-semibold text-base md:text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200", LearningApp.Styles.SlideUp],
                        ...
                        menuView.Button(["default", "justify-start w-full py-3.5 md:py-4 rounded-2xl font-medium text-[#1a1a1a] bg-white/80 hover:bg-white border border-gray-200/60 hover:border-gray-300/80 hover:shadow-sm transition-all duration-200"],
                        ...
                        menuView.Button(["default", $"justify-start w-full py-3.5 md:py-4 rounded-2xl font-semibold {LearningApp.Styles.GetAccentText(theme)} bg-white/60 hover:bg-white/90 border-2 {LearningApp.Styles.GetAccentBorder(theme)} hover:shadow-md transition-all duration-200"],
                            text: $"💬  Chat with Aino",
            ...
            view.Button(["default", "justify-start w-full py-3.5 rounded-2xl font-medium text-[#6b7280] hover:text-[#1a1a1a] bg-white/40 hover:bg-white/70 transition-all duration-200"],
