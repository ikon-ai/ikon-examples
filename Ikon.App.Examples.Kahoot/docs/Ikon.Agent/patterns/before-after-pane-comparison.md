...
A two-column row inside the work area: left pane shows the user's input image (Rhino render, sketch, photo) with a "replace / clear" affordance; right pane shows the AI-generated output with download / show-prompt actions. Both panes use the same square aspect frame, same ring/background, so visual diff is honest. When either pane has no content, it shows a dashed dropzone or a soft italic note instead.
...
- When the output is empty, an italic single-paragraph hint reads better than a second dropzone — it acknowledges the user can't generate yet.
