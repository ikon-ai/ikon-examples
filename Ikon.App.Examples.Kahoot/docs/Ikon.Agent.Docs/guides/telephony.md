...
Send SMS and place phone calls via `app.Telephony`, from a phone number the platform holds for the app's space — no provider account, API key or contract in the app. The space needs a number first (`ikon app telephony create --country se`), which rents monthly and is charged to the app's credits. An app may hold several, in different markets and on different providers.

Recipients are always bare E.164 (`+358401234567`). Check `SmsSendResult.Replyable`: a number that is not local to the recipient's market arrives from an unknown sender and cannot be answered, which no error reports. Calls carry a platform duration limit and are hung up when it is reached.

Omit `from:` and the platform picks the sender — the app's default number, else one local to the recipient. Name it to send as a particular number; `GetNumbersAsync` lists them. Each number delivers its inbound to its own session identity (`ikon app telephony bind --number … --identity …`), which is how one app answers as several users.
