...
- `[App]` attribute - Mandatory, must appear exactly once. Marks the class whose `Main()` method will be executed
- `IApp<SessionIdentity, ClientParameters> host` - Must specify types for SessionIdentity and ClientParameters
- `private UI UI { get; } = new(host, new Theme());` - Initialize UI with host and theme
...
- Available in app via `host.SessionIdentity`
...
- Available via `host.Clients[clientId].Parameters`
...
```csharp
host.StartingAsync += async args => { /* app starting */ };
host.StoppingAsync += async args => { /* app stopping, cleanup */ };
host.ClientJoinedAsync += async args =>
{
    ...
    var client = host.Clients[args.ClientSessionId];
};
host.ClientLeftAsync += async args => { /* cleanup client state */ };
```
...
await host.Navigation.SetPathAsync(args.ClientSessionId, $"/{tab}", replace: true);
```

### App Data and Public Folders

- **`app/Data/` folder**: Files placed here are included in the app bundle. Available at runtime via `host.DataDirectory` for reading from the C# app
...
```csharp
host.GlobalState.SpaceId           // Current space ID
host.GlobalState.ChannelId         // Current channel ID
host.GlobalState.SessionId         // Server session ID
host.GlobalState.ChannelUrl        // Channel access URL
host.GlobalState.SessionChannelUrl // Session-specific access URL
host.GlobalState.PrimaryUserId     // Static user ID of session owner
host.GlobalState.FirstUserId       // First human user who joined (dynamically reassigned)
host.GlobalState.GetClientContext(clientSessionId)  // Get client context
host.DataDirectory                 // Path to app's Data directory
host.Endpoints                     // Custom endpoint info (see Endpoints section)
host.Databases                     // Database connection info (see Databases section)
host.SessionIdentity               // Current session identity
host.Clients[clientId].Parameters  // Client parameters
host.ReactiveGlobalState.Clients   // Reactive client state
```
...
```csharp
await using var work = await host.BackgroundWork.StartAsync();
...
```csharp
host.MessageReceivedAsync += async args => { /* args.Message.Opcode, args.Message.TrackId */ };
