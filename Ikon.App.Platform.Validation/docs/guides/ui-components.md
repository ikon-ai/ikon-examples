...
All UI components are called as methods on `UIView` (the `view` parameter in content lambdas). Style is always the first parameter, using an array of theme constants and/or Crosswind classes.
...
view.Tabs(value: _tab.Value, onValueChange: async v =>
    {
        _tab.Value = v ?? "home";
        await host.Navigation.SetPathAsync($"/{v}");
    },
