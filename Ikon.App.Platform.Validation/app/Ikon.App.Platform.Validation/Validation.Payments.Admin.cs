...
public partial class Validation
{
    ...
    {
        var payouts = _paymentsEventLog.Value
    ...
    private void RenderIdentityPanel(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: card =>
        {
            ...
            card.Text([Text.BodySm, "text-tertiary mb-3"], "Who Stripe thinks this app is, and how the demo user maps to a Stripe customer. Connect mode → one Stripe sub-account per app; BYOK → one Stripe account per app. The `app-customer-key` is whatever stable id the app passes to IPaymentsAppAdapter — typically the Ikon user id or org id in real apps; here a fixed demo string.");

            var apiKey = PaymentsAppHelpers.GetSecretOrEnv(app, "STRIPE_API_KEY");
            ...
            {
                IdentityRow(rows, "provider",            _paymentsProviderValue.ToString());
                IdentityRow(rows, "space",               space);
        ...
        view.Box([Card.Default, "p-6"], content: card =>
        {
            ...
            card.Column([Layout.Column.Md], content: col =>
            {
                col.Column([Layout.Column.Xs], content: g =>
                {
                    ...
                    g.Box(["p-2 rounded-md bg-secondary/40 border border-secondary text-xs font-mono break-all"], content: u => u.Text([], ResolvePaymentsWebhookUrl()));
                });
        ...
        view.Box([Card.Default, "p-6"], content: card =>
        {
            ...
            ActionRow(card, "Create plan", "Product + monthly recurring price in one call. Added live to PricingTable.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Create plan", "Create plan: done", async () =>
                    {
                        ...
                        var productId = await ActivePayments!.CreateProductAsync(new PaymentsProductInfo
                        {
                            Name = name,
                        ...
                        var priceId = await ActivePayments.CreatePriceAsync(new PaymentsPriceInfo
                        {
                            ProductId = productId,
                        ...
                        _paymentsActionStatus.Value = $"Plan created → product={productId} · price={priceId} · planId={lookupKey}";
                    }));
            });
            ...
            ActionRow(card, "List products", "Read all products from Stripe (active + archived).", body: r =>
            {
                r.Button(
                    ...
                    {
                        var products = await ActivePayments!.ListProductsAsync();
                        _adminListedProducts.Value = products;
                        _paymentsActionStatus.Value = $"Loaded {products.Count} products";
                    }));
            });
            ...
            ActionRow(card, "Archive product", "Sets active=false. Existing subscribers unaffected.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Archive product", "Archive: done", async () =>
                    {
                        ...
                        await ActivePayments!.UpdateProductAsync(pid, active: false);
                        _adminProductIdToArchive.Value = "";
                        await RefreshAdminProductsAsync();
                        await RefreshCatalogAsync();
                        _paymentsActionStatus.Value = $"Archived product {pid}";
                    }));
            });
        ...
        view.Box([Card.Default, "p-6"], content: card =>
        {
            ...
            ActionRow(card, "Create customer", "New Stripe customer tagged with `validation_admin_created` metadata.", body: r =>
            {
                ...
                r.Button(
                    ...
                    {
                        var id = await ActivePayments!.CreateCustomerAsync(new PaymentsCustomerInfo
                        {
                        ...
                        try
                        {
                            _adminListedCustomers.Value = await ActivePayments.SearchCustomersDetailedAsync("metadata['validation_admin_created']:'true'");
                        }
                        catch { _adminListedCustomers.Value = [id, .._adminListedCustomers.Value]; }
                        _paymentsActionStatus.Value = $"Customer created → {id}";
                    }));
            });

            ActionRow(card, "Update customer name", "Real admin op — customer asks to change displayed name. Wraps UpdateCustomerAsync with a partial PaymentsCustomerInfo.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Update customer", "Update customer: done", async () =>
                    {
                        ...
                        await ActivePayments!.UpdateCustomerAsync(cid, new PaymentsCustomerInfo { Name = newName });
                        _adminCustomerName.Value = "";
                        _paymentsActionStatus.Value = $"Customer {cid} name → {newName}";
                    }));
            });
            ...
            ActionRow(card, "Search customer", "Find a Stripe customer by `app_customer_key` metadata tag. This is how an app maps its own user/org ids back to Stripe customers — see how `ResolveStripeCustomerIdAsync` in the adapter persists the link.", body: r =>
            {
                r.Button(
                    ...
                    {
                        var rows = await ActivePayments!.SearchCustomersDetailedAsync($"metadata['validation_customer_key']:'{PaymentsDemoCustomerKey}'");
                        _adminListedCustomers.Value = rows;
                        _paymentsActionStatus.Value = $"Customer search → {rows.Count} match";
                    }));
            });
            ...
            ActionRow(card, "Attach tax id (B2B)", "EU VAT or US EIN. Type examples: `eu_vat`, `us_ein`. Required for net-30 invoicing.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Attach tax id", "Attach tax id: done", async () =>
                    {
                        ...
                        var tax = await ActivePayments!.CreateCustomerTaxIdAsync(cid, type, value);
                        _adminTaxIdValue.Value = "";
                        _adminListedTaxIds.Value = await ActivePayments.ListCustomerTaxIdsAsync(cid);
                        _paymentsActionStatus.Value = $"Tax id attached → {tax.Id}";
                    }));
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("List tax ids", "List tax ids: done", async () =>
                    {
                        ...
                        _adminListedTaxIds.Value = await ActivePayments!.ListCustomerTaxIdsAsync(cid);
                        _paymentsActionStatus.Value = $"Loaded {_adminListedTaxIds.Value.Count} tax ids for cus {cid}";
                    }));
            });
            ...
            ActionRow(card, "Delete tax id", "Detach a tax id from its customer. Requires the customer id field above to be set.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Delete tax id", "Delete tax id: done", async () =>
                    {
                        ...
                        await ActivePayments!.DeleteCustomerTaxIdAsync(cid, txid);
                        _adminTaxIdToDelete.Value = "";
                        _adminListedTaxIds.Value = await ActivePayments.ListCustomerTaxIdsAsync(cid);
                        _paymentsActionStatus.Value = $"Tax id {txid} deleted from {cid}";
                    }));
            });
            ...
            ActionRow(card, "Create coupon", "Server-side discount (one-time). Customer never sees the coupon id — promo codes wrap it.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Create coupon", "Create coupon: done", async () =>
                    {
                        ...
                        var id = await ActivePayments!.CreateCouponAsync(new PaymentsCouponInfo
                        {
                            Id = $"VAL_{DateTimeOffset.UtcNow:yyyyMMddHHmmss}",
                            Name = $"Validation {pct}%",
                            PercentOff = pct,
                            Duration = PaymentsCouponDuration.Once,
                        });
                        _adminListedCoupons.Value = [id, .._adminListedCoupons.Value];
                        _paymentsActionStatus.Value = $"Coupon created → {id}";
                    }));
                ...
                r.Button(
                    ...
                    {
                        var ids = await ActivePayments!.ListCouponsAsync();
                        _adminListedCoupons.Value = ids;
                        _paymentsActionStatus.Value = $"Loaded {ids.Count} coupons";
                    }));
            });
            ...
            ActionRow(card, "Create promo code", "Customer-facing redemption code. Bind to a coupon id from the list above.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Create promo code", "Create promo code: done", async () =>
                    {
                        ...
                        var id = await ActivePayments!.CreatePromotionCodeAsync(couponId: couponId, code: code);
                        _adminPromoCode.Value = "";
                        _adminListedPromoCodes.Value = [id, .._adminListedPromoCodes.Value];
                        _paymentsActionStatus.Value = $"Promo code {code} → {id}";
                    }));
                ...
                r.Button(
                    ...
                    {
                        var ids = await ActivePayments!.ListPromotionCodesAsync();
                        _adminListedPromoCodes.Value = ids;
                        _paymentsActionStatus.Value = $"Loaded {ids.Count} promo codes";
                    }));
            });
    ...
    {
        if (ActivePayments is null) { return; }
        try
        {
            _adminListedProducts.Value = await ActivePayments.ListProductsAsync();
        }
        catch
        {
        }
            ...
            ActionRow(card, "Hosted invoice", "B2B net-30 — customer receives an emailed Stripe-hosted invoice link to pay.", body: r =>
            {
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Hosted invoice", "Hosted invoice: done", async () =>
                    {
                        ...
                        var inv = await ActivePayments!.CreateHostedInvoiceAsync(
                            stripeCustomerId: ActiveCustomerId,
                            lines: [PaymentsLineItem.Dynamic(500, "eur", "Validation hosted invoice")],
                            daysUntilDue: 30,
                            autoSend: false);
                        _paymentsActionStatus.Value = $"Hosted invoice → {inv.Id} · {inv.HostedInvoiceUrl}";
            ...
            ActionRow(card, "Preview upcoming invoice", "What Stripe will bill the customer next cycle. Requires an active subscription.", body: r =>
            {
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Preview upcoming invoice", "Preview: done", async () =>
                    {
                        ...
                        var preview = await ActivePayments!.PreviewUpcomingInvoiceAsync(ActiveCustomerId, subscriptionId: sub.Id);
                        _paymentsActionStatus.Value = $"Upcoming: {preview.AmountDueMinor / 100m:0.00} {preview.Currency.ToUpperInvariant()} · {preview.Lines.Count} line(s)";
                    }));
            });
            ...
            ActionRow(card, "Create credit note", "Formal partial refund — Stripe handles tax reversal + regenerates the invoice PDF.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Create credit note", "Credit note: done", async () =>
                    {
                        ...
                        var note = await ActivePayments!.CreateCreditNoteAsync(new PaymentsCreditNoteInfo
                        {
                            InvoiceId = invId,
                            AmountMinor = amt,
                        });
                        _adminCreditAmount.Value = "";
                        _adminListedCreditNotes.Value = await ActivePayments.ListCreditNotesAsync();
                        _paymentsActionStatus.Value = $"Credit note → {note.Id} · {note.Status}";
                        await RefreshPaymentsDataAsync();
                    }));
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("List credit notes", "List credit notes: done", async () =>
                    {
                        ...
                        _adminListedCreditNotes.Value = await ActivePayments!.ListCreditNotesAsync(string.IsNullOrEmpty(invFilter) ? null : invFilter);
                        _paymentsActionStatus.Value = $"Loaded {_adminListedCreditNotes.Value.Count} credit notes";
                    }));
            });
            ...
            ActionRow(card, "Void credit note", "Reverse a mistakenly-issued credit note.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Void credit note", "Void credit note: done", async () =>
                    {
                        ...
                        await ActivePayments!.VoidCreditNoteAsync(id);
                        _adminCreditNoteIdToVoid.Value = "";
                        _adminListedCreditNotes.Value = await ActivePayments.ListCreditNotesAsync();
                        _paymentsActionStatus.Value = $"Voided credit note {id}";
                    }));
            });
            ...
            ActionRow(card, "Register endpoint", "Stripe returns a signing secret only once at creation — captured server-side.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Register webhook endpoint", "Register: done", async () =>
                    {
                        ...
                        var ep = await ActivePayments!.CreateWebhookEndpointAsync(
                            url: url,
                            enabledEvents: ["invoice.paid", "customer.subscription.updated", "payment_intent.succeeded"]);
                        _adminWebhookUrl.Value = "";
                        _adminListedWebhookEndpoints.Value = await ActivePayments.ListWebhookEndpointsAsync();
                        _paymentsActionStatus.Value = $"Webhook endpoint → {ep.Id} · secret captured server-side";
                    }));
            });
            ...
            ActionRow(card, "Ping / list / delete endpoints", "Send a v2 test event (Stripe POSTs a synthetic `v2.core.event_destination.ping` to verify wire). Inspect existing endpoints or remove one.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Ping webhook endpoint", "Ping: sent", async () =>
                    {
                        ...
                        await ActivePayments!.PingWebhookEndpointAsync(id);
                        _paymentsActionStatus.Value = $"Pinged endpoint {id} — check the event log for the synthetic ping delivery";
                    }));
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Delete webhook endpoint", "Delete webhook: done", async () =>
                    {
                        ...
                        await ActivePayments!.DeleteWebhookEndpointAsync(id);
                        _adminWebhookIdToDelete.Value = "";
                        _adminListedWebhookEndpoints.Value = await ActivePayments.ListWebhookEndpointsAsync();
                        _paymentsActionStatus.Value = $"Deleted endpoint {id}";
                    }));
                ...
                r.Button(
                    ...
                    {
                        _adminListedWebhookEndpoints.Value = await ActivePayments!.ListWebhookEndpointsAsync();
                        _paymentsActionStatus.Value = $"Loaded {_adminListedWebhookEndpoints.Value.Count} webhook endpoints";
                    }));
            });
            ...
            ActionRow(card, "Adjust customer balance", "Goodwill credit (negative = reduces future invoices). Persists on Stripe customer.", body: r =>
            {
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Adjust customer balance", "Balance adjust: done", async () =>
                    {
                        ...
                        await ActivePayments!.AdjustCustomerBalanceAsync(ActiveCustomerId, -500, "eur", "Validation goodwill credit", $"validation-credit-{Guid.NewGuid():N}");
                        _paymentsActionStatus.Value = "Customer balance adjusted by −€5";
                    }));
            });
            ...
            ActionRow(card, "Replay events (outage recovery)", "List Stripe events from the last 24h and re-deliver to the adapter. Use after a webhook outage.", body: r =>
            {
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Replay events", "Replay: done", async () =>
                    {
                        ...
                        var ids = await ActivePayments!.ListEventIdsAsync(createdAfter: since, limit: 25);
                        ...
                        foreach (var evtId in ids.Take(25))
                        {
                            try
                            {
                                var evt = await ActivePayments.RetrieveEventAsync(evtId);
                                summaries.Add(FormatReplayEvent(evt));

                                if (summaries.Count <= 10)
                                {
                                    LogPaymentsEvent($"[replay] {evt.Type} · {evt.EventId}");
                                }
                            }
                            catch (Exception ex)
                            {
                        ...
                        _paymentsActionStatus.Value = $"Loaded {summaries.Count} recent events";
                    }));
            });
            ...
            ActionRow(card, "Apple Pay domain", "Register a domain so Apple Pay shows up in Checkout. Stripe handles the verification file.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Register Apple Pay domain", "Apple Pay register: done", async () =>
                    {
                        ...
                        var id = await ActivePayments!.RegisterApplePayDomainAsync(d);
                        _adminApplePayDomain.Value = "";
                        _adminListedApplePayDomains.Value = await ActivePayments.ListApplePayDomainsAsync();
                        _paymentsActionStatus.Value = $"Apple Pay domain registered → {id}";
                    }));
                ...
                r.Button(
                    ...
                    {
                        _adminListedApplePayDomains.Value = await ActivePayments!.ListApplePayDomainsAsync();
                        _paymentsActionStatus.Value = $"Loaded {_adminListedApplePayDomains.Value.Count} Apple Pay domains";
                    }));
            });
            ...
            ActionRow(card, "Payment link", "Shareable Stripe-hosted shopping URL — distribute via chat/email/QR. No customer required.", body: r =>
            {
                ...
                r.Button(
                    ...
                    onClick: async () => await RunActionAsync("Create payment link", "Payment link: done", async () =>
                    {
                        ...
                        var link = await ActivePayments!.CreatePaymentLinkAsync(
                            lines: [PaymentsLineItem.ForPrice(priceId, quantity: 1)]);
                        _adminPaymentLinkPriceId.Value = "";
                        _adminListedPaymentLinks.Value = await ActivePayments.ListPaymentLinksAsync();
                        _paymentsActionStatus.Value = $"Payment link → {link.Url}";
                    }));
                ...
                r.Button(
                    ...
                    {
                        _adminListedPaymentLinks.Value = await ActivePayments!.ListPaymentLinksAsync();
                        _paymentsActionStatus.Value = $"Loaded {_adminListedPaymentLinks.Value.Count} payment links";
                    }));
            });
    ...
    private void RenderEventLogSection(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: card =>
        {
            ...
            var log = _paymentsEventLog.Value;
    ...
    /// </summary>
    private static string FormatReplayEvent(PaymentsEvent evt)
    {
