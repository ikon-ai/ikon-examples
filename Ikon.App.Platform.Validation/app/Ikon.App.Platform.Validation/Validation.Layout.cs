public partial class Validation
{
    private void RenderLayoutSection(UIView view)
    {
        ...
        {
            // Raw layout primitives (flex / grid / position / overflow)
            RenderLayoutFlexAlignmentCard(view);
            RenderLayoutFlexItemCard(view);
            RenderLayoutGridCard(view);
            RenderLayoutPositionCard(view);
            RenderLayoutOverflowCard(view);

            // Layout structure & components
            RenderLayoutSeparatorCard(view);
            RenderLayoutAspectRatioCard(view);
            RenderLayoutSplitCard(view);
            RenderLayoutResizableSplitCard(view);

            // Scrolling containers
            RenderLayoutScrollAreaCard(view);
            RenderLayoutInfiniteScrollCard(view);
            RenderLayoutAutoScrollCard(view);

            // Content & media
            RenderLayoutImageCard(view);
            RenderLayoutAvatarSizesCard(view);
            RenderLayoutAvatarShapesCard(view);

            // State & feedback
            RenderLayoutProgressCard(view);
            RenderLayoutSkeletonCard(view);

            // Context / advanced
            RenderLayoutDirectionProviderCard(view);

            // Dev tests & renderer validation
            RenderLayoutBoxOnClickCard(view);
            RenderLayoutFlutterMappingCard(view);
        });
    }

    // Caption + demo pairing used by the raw layout-primitive cards.
    private void LayoutCaptioned(UIView view, string caption, Action<UIView> content)
    {
    ...
    private void RenderLayoutFlexAlignmentCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
            view.Text([Text.H2, "mb-1"], "Flexbox Alignment");
            view.Text([Text.Caption, "mb-4"], "justify-content (main axis) and items / align-items (cross axis) on a flex container.");

            view.Column([Layout.Column.Md], content: view =>
            {
                view.Text([Text.BodyStrong], "justify-content");
                foreach (var j in new[] { "justify-start", "justify-center", "justify-end", "justify-between", "justify-around", "justify-evenly" })
                {
                    var justify = j;
                    LayoutCaptioned(view, justify, v =>
                    {
                        v.Box(["flex w-full bg-surface rounded-md p-2 border border-secondary", justify], content: row =>
                        {
                            row.Box(["w-10 h-6 rounded bg-brand-solid"]);
                            row.Box(["w-10 h-6 rounded bg-brand-solid"]);
                            row.Box(["w-10 h-6 rounded bg-brand-solid"]);
                        });
                    });
                ...
                view.Text([Text.BodyStrong, "mt-4"], "items (align-items, cross axis)");
                foreach (var a in new[] { "items-start", "items-center", "items-end", "items-stretch" })
                {
                    var align = a;
                    var stretch = align == "items-stretch";
                    LayoutCaptioned(view, align, v =>
                    {
                        v.Box(["flex gap-2 h-20 w-full bg-surface rounded-md p-2 border border-secondary", align], content: row =>
                        {
                            row.Box([stretch ? "w-12 rounded bg-brand-solid" : "w-12 h-6 rounded bg-brand-solid"]);
                            row.Box([stretch ? "w-12 rounded bg-brand-solid" : "w-12 h-12 rounded bg-brand-solid"]);
                            row.Box([stretch ? "w-12 rounded bg-brand-solid" : "w-12 h-8 rounded bg-brand-solid"]);
                        });
                    });
                }
            });
    ...
    private void RenderLayoutFlexItemCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
            view.Text([Text.H2, "mb-1"], "Flex Item Sizing");
            view.Text([Text.Caption, "mb-4"], "grow / shrink / basis / order on flex children.");

            view.Column([Layout.Column.Md], content: view =>
            {
                LayoutCaptioned(view, "basis-1/4 · basis-1/2 · basis-1/4", v =>
                {
                    v.Box(["flex gap-2 w-full"], content: row =>
                    {
                        row.Box(["basis-1/4 h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "1/4"));
                        row.Box(["basis-1/2 h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "1/2"));
                        row.Box(["basis-1/4 h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "1/4"));
                    });
                });

                LayoutCaptioned(view, "grow vs grow-0", v =>
                {
                    v.Box(["flex gap-2 w-full"], content: row =>
                    {
                        row.Box(["grow h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "grow"));
                        row.Box(["grow-0 w-24 h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "grow-0 w-24"));
                    });
                });

                LayoutCaptioned(view, "order (source 3·1·2 → renders 1·2·3)", v =>
                {
                    v.Box(["flex gap-2 w-full"], content: row =>
                    {
                        row.Box(["order-3 grow h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "order-3"));
                        row.Box(["order-1 grow h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "order-1"));
                        row.Box(["order-2 grow h-8 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "order-2"));
                    });
                });
            });
        });
    ...
    private void RenderLayoutGridCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
            view.Text([Text.H2, "mb-1"], "CSS Grid");
            view.Text([Text.Caption, "mb-4"], "grid-cols / grid-rows / col-span / row-span / grid-flow with gap-x and gap-y.");

            view.Column([Layout.Column.Md], content: view =>
            {
                LayoutCaptioned(view, "grid-cols-3 gap-2", v =>
                {
                    v.Box(["grid grid-cols-3 gap-2 w-full"], content: g =>
                    {
                        for (var i = 1; i <= 6; i++)
                        {
                            var n = i;
                            g.Box(["h-10 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], n.ToString()));
                        }
                    });
                });

                LayoutCaptioned(view, "col-span-2 (first cell spans two columns)", v =>
                {
                    v.Box(["grid grid-cols-3 gap-2 w-full"], content: g =>
                    {
                        g.Box(["col-span-2 h-10 rounded bg-brand-solid flex items-center justify-center"], content: c => c.Text([Text.Caption], "col-span-2"));
                        g.Box(["h-10 rounded bg-surface border border-secondary"]);
                        g.Box(["h-10 rounded bg-surface border border-secondary"]);
                        g.Box(["h-10 rounded bg-surface border border-secondary"]);
                    });
                });

                LayoutCaptioned(view, "grid-rows-2 + row-span-2", v =>
                {
                    v.Box(["grid grid-cols-3 grid-rows-2 gap-2 w-full h-32"], content: g =>
                    {
                        g.Box(["row-span-2 rounded bg-brand-solid flex items-center justify-center"], content: c => c.Text([Text.Caption], "row-span-2"));
                        g.Box(["rounded bg-surface border border-secondary"]);
                        g.Box(["rounded bg-surface border border-secondary"]);
                        g.Box(["rounded bg-surface border border-secondary"]);
                        g.Box(["rounded bg-surface border border-secondary"]);
                    });
                ...
                LayoutCaptioned(view, "gap-x-6 gap-y-2 (distinct column / row gaps)", v =>
                {
                    v.Box(["grid grid-cols-3 gap-x-6 gap-y-2 w-full"], content: g =>
                    {
                        for (var i = 1; i <= 6; i++)
                        {
                            g.Box(["h-8 rounded bg-surface border border-secondary"]);
                        }
                    });
                });

                LayoutCaptioned(view, "grid-flow-col auto-cols-fr (items flow into columns)", v =>
                {
                    v.Box(["grid grid-flow-col auto-cols-fr gap-2 w-full"], content: g =>
                    {
                        g.Box(["h-10 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "A"));
                        g.Box(["h-10 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "B"));
                        g.Box(["h-10 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], "C"));
                    });
                });
            });
        });
    ...
    private void RenderLayoutPositionCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
            view.Text([Text.H2, "mb-1"], "Position & Z-Index");
            view.Text([Text.Caption, "mb-4"], "relative / absolute with inset, sticky inside a scroll area, and z-index stacking.");

            view.Column([Layout.Column.Md], content: view =>
            {
                LayoutCaptioned(view, "relative + absolute (corners via top/right/bottom/left, center via inset-0)", v =>
                {
                    v.Box(["relative h-32 w-full bg-surface rounded-md border border-secondary"], content: box =>
                    {
                        box.Box(["absolute top-2 left-2 px-2 py-1 rounded bg-brand-solid"], content: c => c.Text([Text.Caption], "top-left"));
                        box.Box(["absolute top-2 right-2 px-2 py-1 rounded bg-brand-solid"], content: c => c.Text([Text.Caption], "top-right"));
                        box.Box(["absolute bottom-2 left-2 px-2 py-1 rounded bg-brand-solid"], content: c => c.Text([Text.Caption], "bottom-left"));
                        box.Box(["absolute bottom-2 right-2 px-2 py-1 rounded bg-brand-solid"], content: c => c.Text([Text.Caption], "bottom-right"));
                        box.Box(["absolute inset-0 flex items-center justify-center"], content: c => c.Text([Text.Caption], "inset-0 centered"));
                    });
                });

                LayoutCaptioned(view, "z-index (z-20 overlaps z-10)", v =>
                {
                    v.Box(["relative h-24 w-full"], content: box =>
                    {
                        box.Box(["absolute top-2 left-2 w-32 h-16 rounded bg-blue-500 z-10 flex items-center justify-center"], content: c => c.Text(["text-white"], "z-10"));
                        box.Box(["absolute top-6 left-16 w-32 h-16 rounded bg-amber-400 z-20 flex items-center justify-center"], content: c => c.Text(["text-zinc-950"], "z-20"));
                    });
                });

                LayoutCaptioned(view, "sticky top-0 (scroll the box — header stays pinned)", v =>
                {
                    v.Box(["h-40 w-full overflow-y-auto rounded-md border border-secondary"], content: box =>
                    {
                        box.Box(["sticky top-0 bg-brand-solid px-3 py-2 z-10"], content: c => c.Text([Text.Caption], "sticky header"));
                        box.Column(["p-3 gap-2"], content: col =>
                        {
                            for (var i = 1; i <= 12; i++)
                            {
                                var n = i;
                                col.Text([Text.Body], $"Scrollable row {n}");
                            }
                        });
                    });
                });
    ...
    private void RenderLayoutOverflowCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
            view.Text([Text.H2, "mb-1"], "Overflow");
            view.Text([Text.Caption, "mb-4"], "Raw overflow utilities, distinct from the ScrollArea component.");

            view.Column([Layout.Column.Md], content: view =>
            {
                LayoutCaptioned(view, "overflow-auto (scrolls when content exceeds the box)", v =>
                {
                    v.Box(["h-24 w-full overflow-auto rounded-md border border-secondary p-3"], content: box =>
                    {
                        box.Column(["gap-1"], content: col =>
                        {
                            for (var i = 1; i <= 10; i++)
                            {
                                var n = i;
                                col.Text([Text.Body], $"Line {n}");
                            }
                        });
                    });
                });

                LayoutCaptioned(view, "overflow-hidden (content is clipped)", v =>
                {
                    v.Box(["h-16 w-full overflow-hidden rounded-md border border-secondary p-3"], content: box =>
                    {
                        box.Column(["gap-1"], content: col =>
                        {
                            for (var i = 1; i <= 10; i++)
                            {
                                var n = i;
                                col.Text([Text.Body], $"Clipped line {n}");
                            }
                        });
                    });
                });

                LayoutCaptioned(view, "overflow-x-auto (horizontal scroll)", v =>
                {
                    v.Box(["w-full overflow-x-auto rounded-md border border-secondary p-3"], content: box =>
                    {
                        box.Box(["flex gap-2 w-max"], content: row =>
                        {
                            for (var i = 1; i <= 12; i++)
                            {
                                var n = i;
                                row.Box(["w-24 h-10 shrink-0 rounded bg-surface border border-secondary flex items-center justify-center"], content: c => c.Text([Text.Caption], $"Card {n}"));
                            }
                        });
                    });
                });
    ...
    private void RenderLayoutSeparatorCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutAspectRatioCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutSplitCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutResizableSplitCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutScrollAreaCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutInfiniteScrollCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutAutoScrollCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutImageCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutAvatarSizesCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutAvatarShapesCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutProgressCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutSkeletonCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutDirectionProviderCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutBoxOnClickCard(UIView view)
    {
        view.Box([Card.Default, "p-6"], content: view =>
        {
    ...
    private void RenderLayoutFlutterMappingCard(UIView view)
    {
        // Crosswind -> Flutter style mapping checks. Niche renderer-validation, kept at the bottom.
        view.Box([Card.Default, "p-6"], content: view =>
        {
