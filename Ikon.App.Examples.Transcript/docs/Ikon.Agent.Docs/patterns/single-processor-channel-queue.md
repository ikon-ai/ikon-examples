...
- Wrap individual command handling in its own try/catch so one bad LLM response doesn't tear down the loop. Catch `OperationCanceledException` and `break` so the reader loop exits and its `finally` still clears the busy flag.
