<!-- mined-from: Sentrix -->
# Weighted Progress Banner — Two Phases, One Percentage

A persistent top banner showing overall completion across two pipeline phases (per-file intake + case-level preprocessing) as a single percentage. Each phase contributes a fixed weight, so the number advances smoothly across the phase boundary instead of jumping back to 0% when phase 2 starts. The banner renders a spinner and the percentage as text — the lesson is the weighting math, not the widget.
...
- `Math.Clamp(totalPct, 1, 99)` prevents two visual lies: 0% looks stalled, 100% looks finished (when in reality the banner is dismissed by a separate "everything cleared" signal).
