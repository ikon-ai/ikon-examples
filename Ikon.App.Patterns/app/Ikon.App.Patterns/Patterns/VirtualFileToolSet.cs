...
internal sealed class VirtualFileToolSet : IPatternDemo
{
    ...
    public string Title => "Virtual file tool set";
    public string Category => "AI";
    public void RenderDemo(IView view) => PatternDemoNote.RenderInfo(view, Title, "An in-memory sandbox file system exposed to an LLM agent as write/read/list/delete/search tools — see docs/patterns/virtual-file-tool-set.md.");

    private enum LogLevel { Info, Event, Error }

    private sealed class ExampleState
    {
        public void Log(string message, LogLevel level) => throw new NotImplementedException();
    }

    private sealed class CoderResponse
    {
        public string Summary { get; set; } = "";
    }

    private readonly ExampleState state = null!;
    private readonly KernelContext ctx = new();
    private readonly string task = "";
    private readonly int maxIterations = 10;
    private readonly CancellationTokenSource? _cts = new();
    private readonly Reactive<string> _selectedFile = new("");

    private object ListFiles(ExampleState state) => throw new NotImplementedException();

    private object DeleteFile(ExampleState state, string path) => throw new NotImplementedException();
