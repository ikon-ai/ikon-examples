...
- `ReactivateIfIdleAsync` replaces any hand-rolled status-transition table: it re-engages a thread paused in Idle/WaitingForInput/Done/Failed and no-ops (returns false) when already Active — and also when the thread is still Pending (only threads created with an explicit `InitialStatus: ThreadStatus.Pending` — the default for freshly created threads is Active): there is no Reactivate arc from Pending, so such a thread's first turn goes through `EnsureActiveAsync` or `DriveAsync`, which handles Pending itself. `DriveAsync(DriveMode.UntilQuiescent)` then runs passes until the thread settles back to Idle; concurrent drives coalesce, so calling it from every send-button press is safe.
- `DriveAsync` returns a `DriveOutcome`, and the two cases are worth telling apart: `Settled` means the thread reached a settling state, while `HitSafetyLimit` means the loop ran out its `safety` pass cap (256 by default) while still making changes every pass — a stuck thread, not a finished one. Both just return, so a caller that ignores the outcome renders a runaway agent as a completed turn.
