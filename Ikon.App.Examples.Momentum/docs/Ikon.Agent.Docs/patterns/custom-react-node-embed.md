...
- The JS side lives under `frontend-node/` and is registered via an `IkonUiComponentResolver` wired up with `registry.registerModule(...)` — see `frontend-fundamentals` guide.
