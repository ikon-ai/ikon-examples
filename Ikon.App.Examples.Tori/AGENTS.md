...
- **`IApp` is a generic interface — ALWAYS pass type arguments.** The canonical app class shape is `[App] public class MyApp(IApp<SessionIdentity, ClientParameters> app)` with `record SessionIdentity(string UserId)` and `record ClientParameters(string Name = "Anonymous")` declared as records (use empty `()` if you have no fields). Writing the primary-constructor param as bare `IApp app` (no type args) produces CS0305 "using the generic type 'IApp<TSessionIdentity, TClientParameters>' requires 2 type arguments". The scaffold's existing app file already has the canonical shape — preserve it; don't rewrite the constructor signature from scratch.
