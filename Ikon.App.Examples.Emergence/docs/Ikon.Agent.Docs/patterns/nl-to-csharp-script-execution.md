...
- Pair with `Emerge.Refine<UICodeResponse>` and `opt.ShouldContinue = async (r, _) => !await IsSyntaxValidAsync(r.Code)` to auto-fix compile errors without bothering the user — `ShouldContinue` is `Func<T, EmergenceTrace, Task<bool>>`, and returning true keeps refining.
